...
* T3.2 implementation; the message stays self-contained AND carries the
* code (the human carrier — the journey machine-matches that text).
*
* 0.1.2-alpha.3 typert train: the message-only carrier gains a structured
* channel. The structural marker (`isDSHRemoteError`) plus `code` plus the
* JSON `details` payload satisfy the typert-protocol `remoteErrorOf` check,
* so the Host Gateway's `rpcFailure` passes the `PLANE_*` code through onto
* the wire unchanged (without the marker the gateway folds the throw to
* `gateway/internal` and the client can no longer machine-match, e.g. the
* bindProject `PLANE_TREE_EXISTS` as-is retry in the research shell).
*/
var PlaneError = class extends Error {
	code;
	/** Structural RemoteError marker (the typert-protocol `remoteErrorOf` gate). */
	isDSHRemoteError = true;
	/** Wire details payload (PURE JSON DTO — the empty object). */
	details = {};
...
const PlaneIntegrityDtoSchema = z.object({
	readOnly: z.boolean(),
	checkCodes: z.array(z.string())
}).strict();
const PlaneProjectDtoSchema = z.object({
	...
	wsPath: absolutePath,
	integrity: PlaneIntegrityDtoSchema.optional()
}).strict();
...
const DependencyEdgeDtoSchema = z.object({
...
const clearBlockerInvocation = descriptor("clearBlocker", [argsParameter("ClearBlockerArgs", ClearBlockerArgsSchema)], "ClearBlockerResult", ClearBlockerResultSchema);
const CreatePlanItemArgsSchema = z.object({
	workstreamId: idWorkstream,
	kind: z.enum([
		"TASK",
		"GATE",
	...
	item: z.union([
		...
		z.object({ gate: z.object({
			title: z.string().min(1),
			criteria: z.string().min(1).optional(),
			references: z.array(z.string().min(1)).optional()
		}).strict() }).strict(),
		z.object({ milestone: z.object({
			title: z.string().min(1),
			statement: z.string().min(1).optional()
		}).strict() }).strict()
	]),
...
const CreatePlanItemResultSchema = z.object({
	itemId: z.union([
		idTask,
		idGate,
		idMilestone
...
const createPlanItemInvocation = descriptor("createPlanItem", [argsParameter("CreatePlanItemArgs", CreatePlanItemArgsSchema)], "CreatePlanItemResult", CreatePlanItemResultSchema);
const UpdatePlanItemArgsSchema = z.object({
	workstreamId: idWorkstream,
	itemId: z.union([
		idTask,
		idGate,
...
const UpdatePlanItemResultSchema = z.object({
	itemId: z.union([
		idTask,
		idGate,
		idMilestone
...
const updatePlanItemInvocation = descriptor("updatePlanItem", [argsParameter("UpdatePlanItemArgs", UpdatePlanItemArgsSchema)], "UpdatePlanItemResult", UpdatePlanItemResultSchema);
const RemovePlanItemArgsSchema = z.object({
	workstreamId: idWorkstream,
	itemId: z.union([
		idTask,
		idGate,
...
const RemovePlanItemResultSchema = z.object({
	workstreamId: idWorkstream,
	planPath: z.string().min(1),
	newOrder: z.array(z.string().min(1)),
	managementActionId: idManagementAction,
...
const removePlanItemInvocation = descriptor("removePlanItem", [argsParameter("RemovePlanItemArgs", RemovePlanItemArgsSchema)], "RemovePlanItemResult", RemovePlanItemResultSchema);
...
const AddDependencyArgsSchema = z.object({
	workstreamId: idWorkstream,
	source: dependencyEndpointSchema,
	target: dependencyEndpointSchema,
	projectId: idProject.optional()
}).strict();
const AddDependencyResultSchema = z.object({
	relationId: idRelation,
	source: dependencyEndpointSchema,
	target: dependencyEndpointSchema
}).strict();
const addDependencyInvocation = descriptor("addDependency", [argsParameter("AddDependencyArgs", AddDependencyArgsSchema)], "AddDependencyResult", AddDependencyResultSchema);
const RemoveDependencyArgsSchema = z.object({
	workstreamId: idWorkstream,
	relationId: idRelation,
	projectId: idProject.optional()
}).strict();
const RemoveDependencyResultSchema = z.object({ relationId: idRelation }).strict();
const removeDependencyInvocation = descriptor("removeDependency", [argsParameter("RemoveDependencyArgs", RemoveDependencyArgsSchema)], "RemoveDependencyResult", RemoveDependencyResultSchema);
const CreateWorkstreamForkArgsSchema = z.object({
	topicId: idTopic,
...
const CreateWorkstreamForkResultSchema = z.object({
	topicId: idTopic,
	edgeIds: z.array(idTopologyEdge).min(1),
	workstreamIds: z.array(idWorkstream).min(1),
	managementActionId: idManagementAction
}).strict();
const createWorkstreamForkInvocation = descriptor("createWorkstreamFork", [argsParameter("CreateWorkstreamForkArgs", CreateWorkstreamForkArgsSchema)], "CreateWorkstreamForkResult", CreateWorkstreamForkResultSchema);
const CreatePlannedMergeArgsSchema = z.object({
	topicId: idTopic,
...
const CreatePlannedMergeResultSchema = z.object({
...
const createPlannedMergeInvocation = descriptor("createPlannedMerge", [argsParameter("CreatePlannedMergeArgs", CreatePlannedMergeArgsSchema)], "CreatePlannedMergeResult", CreatePlannedMergeResultSchema);
const GetMergeContractArgsSchema = z.object({
	edgeId: idTopologyEdge,
	projectId: idProject.optional()
}).strict();
const GetMergeContractResultSchema = z.object({
	edgeId: idTopologyEdge,
	content: z.string().nullable(),
	path: z.string().min(1)
}).strict();
const getMergeContractInvocation = descriptor("getMergeContract", [argsParameter("GetMergeContractArgs", GetMergeContractArgsSchema)], "GetMergeContractResult", GetMergeContractResultSchema);
const SaveMergeContractArgsSchema = z.object({
	edgeId: idTopologyEdge,
	content: z.string().min(1),
	projectId: idProject.optional()
}).strict();
const SaveMergeContractResultSchema = z.object({
	edgeId: idTopologyEdge,
	path: z.string().min(1),
	managementActionId: idManagementAction
}).strict();
const saveMergeContractInvocation = descriptor("saveMergeContract", [argsParameter("SaveMergeContractArgs", SaveMergeContractArgsSchema)], "SaveMergeContractResult", SaveMergeContractResultSchema);
const DropTopologyEdgeArgsSchema = z.object({
	edgeId: idTopologyEdge,
	projectId: idProject.optional()
}).strict();
const DropTopologyEdgeResultSchema = z.object({
	edgeId: idTopologyEdge,
	topicId: idTopic,
	lifecycle: z.literal("DROPPED"),
	managementActionId: idManagementAction
}).strict();
const dropTopologyEdgeInvocation = descriptor("dropTopologyEdge", [argsParameter("DropTopologyEdgeArgs", DropTopologyEdgeArgsSchema)], "DropTopologyEdgeResult", DropTopologyEdgeResultSchema);
...
const semanticEndpointSchema = z.object({
...
const RecordFactArgsSchema = z.object({
	workstreamId: idWorkstream,
	statement: z.string().min(1),
	references: z.array(recordReferenceSchema).optional(),
	projectId: idProject.optional()
}).strict();
const RecordFactResultSchema = z.object({
	factId: idFact,
	workstreamId: idWorkstream,
	statement: z.string(),
...
const recordFactInvocation = descriptor("recordFact", [argsParameter("RecordFactArgs", RecordFactArgsSchema)], "RecordFactResult", RecordFactResultSchema);
const RecordClaimArgsSchema = z.object({
	workstreamId: idWorkstream,
	statement: z.string().min(1),
	references: z.array(recordReferenceSchema).optional(),
	projectId: idProject.optional()
}).strict();
const RecordClaimResultSchema = z.object({
	claimId: idClaim,
	workstreamId: idWorkstream,
	statement: z.string(),
	references: z.array(recordReferenceSchema),
...
const recordClaimInvocation = descriptor("recordClaim", [argsParameter("RecordClaimArgs", RecordClaimArgsSchema)], "RecordClaimResult", RecordClaimResultSchema);
const RetractClaimArgsSchema = z.object({
	claimId: idClaim,
	reason: z.string().min(1).optional(),
	projectId: idProject.optional()
}).strict();
const RetractClaimResultSchema = z.object({
	claimId: idClaim,
	status: z.literal("RETRACTED"),
	eventId: idHistoryEvent
}).strict();
const retractClaimInvocation = descriptor("retractClaim", [argsParameter("RetractClaimArgs", RetractClaimArgsSchema)], "RetractClaimResult", RetractClaimResultSchema);
const RegisterArtifactArgsSchema = z.object({
	workstreamId: idWorkstream,
...
const RegisterArtifactResultSchema = z.object({
...
const registerArtifactInvocation = descriptor("registerArtifact", [argsParameter("RegisterArtifactArgs", RegisterArtifactArgsSchema)], "RegisterArtifactResult", RegisterArtifactResultSchema);
const MarkArtifactMissingArgsSchema = z.object({
	artifactId: idArtifact,
	reason: z.string().min(1).optional(),
	projectId: idProject.optional()
}).strict();
const MarkArtifactMissingResultSchema = z.object({
	artifactId: idArtifact,
	status: z.literal("MISSING"),
	eventId: idHistoryEvent
}).strict();
const markArtifactMissingInvocation = descriptor("markArtifactMissing", [argsParameter("MarkArtifactMissingArgs", MarkArtifactMissingArgsSchema)], "MarkArtifactMissingResult", MarkArtifactMissingResultSchema);
const AddRelationArgsSchema = z.object({
	source: semanticEndpointSchema,
	relationType: relationTypeSchema,
	target: semanticEndpointSchema,
	projectId: idProject.optional()
}).strict();
const AddRelationResultSchema = z.object({
	relationId: idRelation,
	source: semanticEndpointSchema,
	relationType: relationTypeSchema,
	target: semanticEndpointSchema,
...
const addRelationInvocation = descriptor("addRelation", [argsParameter("AddRelationArgs", AddRelationArgsSchema)], "AddRelationResult", AddRelationResultSchema);
const RemoveRelationArgsSchema = z.object({
	relationId: idRelation,
	reason: z.string().min(1).optional(),
	projectId: idProject.optional()
}).strict();
const RemoveRelationResultSchema = z.object({
	relationId: idRelation,
	status: z.literal("REMOVED"),
	eventId: idHistoryEvent
}).strict();
const removeRelationInvocation = descriptor("removeRelation", [argsParameter("RemoveRelationArgs", RemoveRelationArgsSchema)], "RemoveRelationResult", RemoveRelationResultSchema);
const semanticRecordDtoSchema = z.object({
	id: z.string(),
	type: z.enum([
		"FACT",
		"CLAIM",
...
const QueryRecordsResultSchema = z.object({
	records: z.array(semanticRecordDtoSchema),
	total: z.number().int().nonnegative()
}).strict();
const queryRecordsInvocation = descriptor("queryRecords", [argsParameter("QueryRecordsArgs", QueryRecordsArgsSchema)], "QueryRecordsResult", QueryRecordsResultSchema);
const attentionItemDtoSchema = z.object({
	...
	context: z.object({
		...
		derivedBlocker: z.object({ primaryAction: z.object({
			label: z.string().min(1),
...
const QueryAttentionArgsSchema = z.object({
	projectId: idProject.optional(),
	workstreamId: idWorkstream.optional(),
	kind: z.enum([
		"INTERVENTION",
...
const QueryAttentionResultSchema = z.object({
...
const RESEARCH_MANAGEMENT_INVOCATIONS = [
	...
	descriptor("queryAttention", [argsParameter("QueryAttentionArgs", QueryAttentionArgsSchema)], "QueryAttentionResult", QueryAttentionResultSchema)
];
/**
...
//#endregion
export { InspectProjectDirectoryResultSchema as $, UpdateInterventionStateResultSchema as $t, DismissPlanForkArgsSchema as A, ReorderPlanResultSchema as At, GetHubOverviewArgsSchema as B, SaveMergeContractResultSchema as Bt, CreateWorkstreamArgsSchema as C, RemoveDependencyArgsSchema as Ct, DashboardSnapshotSchema as D, RemoveRelationArgsSchema as Dt, CreateWorkstreamResultSchema as E, RemovePlanItemResultSchema as Et, DropWorkstreamResultSchema as F, RestoreProjectArgsSchema as Ft, GetResearchPlaneStateArgsSchema as G, SetCurrentFocusArgsSchema as Gt, GetMergeContractResultSchema as H, SaveResearchCheckpointResultSchema as Ht, GetCurrentFocusArgsSchema as I, RestoreProjectResultSchema as It, GetWorkstreamArgsSchema as J, SetHubResultSchema as Jt, GetResearchPlaneStateResultSchema as K, SetCurrentFocusResultSchema as Kt, GetCurrentFocusResultSchema as L, RetractClaimArgsSchema as Lt, DropTopologyEdgeArgsSchema as M, RescanResultSchema as Mt, DropTopologyEdgeResultSchema as N, RestoreDeclarativeFileArgsSchema as Nt, DismissNextActionArgsSchema as O, RemoveRelationResultSchema as Ot, DropWorkstreamArgsSchema as P, RestoreDeclarativeFileResultSchema as Pt, InspectProjectDirectoryArgsSchema as Q, UpdateInterventionStateArgsSchema as Qt, GetGitHistoryArgsSchema as R, RetractClaimResultSchema as Rt, CreateTopicResultSchema as S, RegisterInteractionResultSchema as St, CreateWorkstreamForkResultSchema as T, RemovePlanItemArgsSchema as Tt, GetPortfolioInterventionsArgsSchema as U, SelectPlanForkArgsSchema as Ut, GetMergeContractArgsSchema as V, SaveResearchCheckpointArgsSchema as Vt, GetPortfolioInterventionsResultSchema as W, SelectPlanForkResultSchema as Wt, GetWorkstreamCurrentResultSchema as X, UnbindProjectArgsSchema as Xt, GetWorkstreamCurrentArgsSchema as Y, TopicSnapshotSchema as Yt, HubOverviewResultSchema as Z, UnbindProjectResultSchema as Zt, CreatePlanItemArgsSchema as _, RecordFactArgsSchema as _t, AddRelationArgsSchema as a, UpdateProjectMetadataResultSchema as an, PromoteNextActionArgsSchema as at, CreatePlannedMergeResultSchema as b, RegisterArtifactResultSchema as bt, BindProjectResultSchema as c, UpdateWorkstreamArgsSchema as cn, QueryAttentionResultSchema as ct, CreateBlockerArgsSchema as d, QueryRecordsArgsSchema as dt, UpdateObjectiveArgsSchema as en, MarkArtifactMissingArgsSchema as et, CreateBlockerResultSchema as f, QueryRecordsResultSchema as ft, CreateNextActionResultSchema as g, RecordClaimResultSchema as gt, CreateNextActionArgsSchema as h, RecordClaimArgsSchema as ht, AddDependencyResultSchema as i, UpdateProjectMetadataArgsSchema as in, ProjectSnapshotSchema as it, DismissPlanForkResultSchema as j, RescanArgsSchema as jt, DismissNextActionResultSchema as k, ReorderPlanArgsSchema as kt, ClearBlockerArgsSchema as l, UpdateWorkstreamResultSchema as ln, QueryHistoryArgsSchema as lt, CreateLocalResearchProjectResultSchema as m, RESEARCH_CONTROL_PACKAGE as mt, AckMissingReminderResultSchema as n, UpdatePlanItemArgsSchema as nn, PingResultSchema as nt, AddRelationResultSchema as o, UpdateTopicArgsSchema as on, PromoteNextActionResultSchema as ot, CreateLocalResearchProjectArgsSchema as p, REGISTERED_RESEARCH_INVOCATIONS as pt, GetTopicArgsSchema as q, SetHubArgsSchema as qt, AddDependencyArgsSchema as r, UpdatePlanItemResultSchema as rn, PlaneError as rt, BindProjectArgsSchema as s, UpdateTopicResultSchema as sn, QueryAttentionArgsSchema as st, AckMissingReminderArgsSchema as t, UpdateObjectiveResultSchema as tn, MarkArtifactMissingResultSchema as tt, ClearBlockerResultSchema as u, WorkstreamSnapshotSchema as un, QueryHistoryResultSchema as ut, CreatePlanItemResultSchema as v, RecordFactResultSchema as vt, CreateWorkstreamForkArgsSchema as w, RemoveDependencyResultSchema as wt, CreateTopicArgsSchema as x, RegisterInteractionArgsSchema as xt, CreatePlannedMergeArgsSchema as y, RegisterArtifactArgsSchema as yt, GetGitHistoryResultSchema as z, SaveMergeContractArgsSchema as zt };
