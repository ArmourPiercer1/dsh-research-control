...
import type { InboxSchemaError, InboxShapeCheck, InboxSchemas } from './types.js'
 ...
 */
export function loadInboxSchemas(reader: ResearchFileReader, schemaDir: string): InboxSchemas {
  const errors: InboxSchemaError[] = []
  ...
  if (doc === null || typeof doc.$id !== 'string') {
    errors.push({ path: pjoin(schemaDir, 'inbox.schema.json'), message: 'inbox.schema.json is missing or has no $id' })
    return unavailable(schemaDir, errors)
  }
  try {
    ajv.addSchema(doc, doc.$id)
  } catch (cause) {
    errors.push({ path: pjoin(schemaDir, 'inbox.schema.json'), message: `inbox.schema.json rejected: ${cause instanceof Error ? cause.message : String(cause)}` })
    return unavailable(schemaDir, errors)
  }

  const recordValidator = ajv.getSchema(`${doc.$id}#/$defs/InboxItem`)
  if (recordValidator === undefined) {
    errors.push({ path: pjoin(schemaDir, 'inbox.schema.json'), message: 'schema compile failed for $defs/InboxItem' })
    return unavailable(schemaDir, errors)
  }
...
function mapErrors(validator: ValidateFunction): InboxSchemaError[] {
  return (validator.errors ?? []).map((err) => ({ path: err.instancePath, message: schemaErrorSummary(err) }))
}

function runCheck(validator: ValidateFunction, value: unknown): InboxShapeCheck {
  const ok = validator(value) as boolean
  if (ok) return { ok: true, errors: [] }
  return { ok: false, errors: mapErrors(validator) }
}

function unavailable(schemaDir: string, errors: readonly InboxSchemaError[]): InboxSchemas {
  const unavailableCheck: InboxShapeCheck = {
    ok: false,
    errors: [{ path: '', message: 'inbox schema set unavailable — see InboxSchemas.loadErrors' }],
  }
  return {
    schemaDir,
    isUsable: false,
