...
const CROSS_PLATFORM_ABSOLUTE_PATH_PATTERN = /^(?:[A-Za-z]:[\\/]|\\|\/)/;
/**
...
function requireAbs(value, name) {
	if (typeof value !== "string" || value.length === 0 || !isAbsolutePath(value)) throw new HardeningError("HARDENING_INPUT", `${name} must be an absolute path (POSIX "/…", Windows drive "C:\\…", or UNC "\\\\…"; got ${JSON.stringify(value ?? null)})`);
	return value;
}
//#endregion
...
function buildInvestigationContext(intervention, question, cwd) {
	...
	if (!isAbsolutePath(cwd)) throw badInput("cwd (must be an absolute path)", cwd);
...
function assertAbsoluteCwd(cwd) {
	if (!isAbsolutePath(cwd)) throw new InvestigatorLaunchError({
		code: "IVL_INPUT",
...
function createHostWiring(options) {
	...
	const requireAbs = (value, name) => {
		if (typeof value !== "string" || value.length === 0 || !isAbsolutePath(value)) throw new HostWiringError("WIRING_INPUT", `${name} must be an absolute path (POSIX "/…", Windows drive "C:\\…", or UNC "\\\\…"; got ${JSON.stringify(value ?? null)})`);
		return value;
	};
