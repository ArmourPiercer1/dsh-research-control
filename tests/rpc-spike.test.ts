...
describe('WP-0.3 ping RPC spike', () => {
  it('① @Remote("ping") marks the method as a direct Remote invocation', () => {
    ...
    // read-only plane RPCs (design §12 rows 1-3); V2-T3.2b: + the 6
    // change-family plane RPCs (design §12 rows 4-6/8/9); UI-0.4: + the 4
    // GUI management RPCs (D §7.2); V2-UI-0.4 UI-2: + the 6 GUI
    ...
    // RPCs; D §14 (UI-8): + the 1 unified attention
    // RPC — the exhaustive 59-method equality
  ...
  it('③ client descriptors and manifest invocations are the same strict contract', () => {
    ...
    // read-only plane RPCs; V2-T3.2b: + the 6 change-family plane RPCs
    // (design §12 rows 4-6/8/9); UI-0.4: + the 4 GUI management RPCs (D
    // §7.2); V2-UI-0.4 UI-2: + the 6 GUI management RPCs (D §8.1/§8.3);
    ...
    // D §14 (UI-8): + the 1 unified attention RPC;
    // the exhaustive 59-descriptor equality is pinned in
    // tests/rpc-face/.
